...
\DIFaddbegin \DIFadd{In this section we show the importance and usefulness of a multifaceted approach to community detection in online social networks. We begin by showing that the communities emerging from the different weightings of the structural network  quantitatively differ both at the macroscopic (}{\it \DIFadd{e.g.,}} \DIFadd{number of communities and their size distribution) and microscopic (}{\it \DIFadd{e.g.,}} \DIFadd{specific memberships, comemberships) scale in interesting ways. Finally, in order to provide a practical illustration of the utility of this question-oriented multifaceted approach, we present several concrete examples of multifaceted community memberships discovered with this method. We will show examples both at the community and individual user level.
}\DIFaddend 
...
\begin{table}[ht]
	...
	\begin{tabular}{| c | c | c |}
		...
		A, Lag 6 & 106 & 642 \\
		T & 289 & 1064 \\
		I & 252 & 2436  \DIFdelbeginFL \DIFdelFL{(1719) }\DIFdelendFL \\ \hline
	\end{tabular}
...
Most importantly, we see that the distributions of community sizes differ across the community types, \DIFdelbegin \DIFdel{highlighting that the different networks give rise to different }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend large-scale community structure \DIFaddbegin \DIFadd{is highly }\DIFaddend dependent on the particular \DIFdelbegin \DIFdel{weighting of the structural network}\DIFdelend \DIFaddbegin \DIFadd{question posed}\DIFaddend .
...
We considered the normalized mutual information between the communities inferred from the structural network and the networks weighted with lag 1 through 6 transfer entropies, hashtag similarity, and mention, retweet, and mention-retweet activity. The resulting $\text{NMI}(\mathcal{C}_{i}, \mathcal{C}_{j})$ are shown in Figure~\ref{Fig-compare_coverings}. \DIFdelbegin \DIFdel{We see that similarity between the coverings is dictated by the generic community type (structural, }\DIFdelend \DIFaddbegin \DIFadd{Note the block diagonal structure in Figure~\ref{Fig-compare_coverings}, which indicates that coverings are most similar within a question category. For example, the }\DIFaddend activity-based \DIFdelbegin \DIFdel{, etc. ). That is, the }\DIFdelend transfer entropy coverings are more similar to each other than to any of the other coverings\DIFdelbegin \DIFdel{, with a similar result for the }\DIFdelend \DIFaddbegin \DIFadd{. Similarly for the interaction-based }\DIFaddend mention, retweet, and mention-retweet coverings. \DIFaddbegin \DIFadd{This point may seem obvious, but the fact that the different weightings within a question category result in similar community structure indicates that each covering is capturing true, latent properties of the social network. }\DIFaddend Interestingly, the coverings resulting from the different weightings are all more similar to each other than to the structural covering from the unweighted network. Also note that the covering based on the hashtag similarities are different from all of the other weight-based coverings. In agreement with the results reported in the previous section, we see that the activity-based communities share similar structure for lags greater than 2. Because of these two results, in the remainder of the paper, we focus on the activity-based communities inferred using the lag-4 transfer entropy.
...
in the context of modularity maximization-based community detection, an exponential number of nearby partitions may exist that nearly maximize an objective function used to measure the goodness-of-fit of a graph partition used for community detection. Because of this and related issues, it is always wise to perform some sort of qualitative study of the communities returned by any  community detectionalgorithm to verify their meaningfulness with respect to the scientific question at hand. In this section, we consider a collection of communities in such a study}\DIFdelend %DIF > As demonstrated by~\cite{good2010performance} in the context of modularity maximization-based community detection, an exponential number of nearby partitions may exist that nearly maximize an objective function used to measure the goodness-of-fit of a graph partition used for community detection. Because of this and related issues, it is always wise to perform some sort of qualitative study of the communities returned by any  community detection algorithm to verify their meaningfulness with respect to the scientific question at hand. In this section, we consider a collection of communities in such a study.
\DIFaddbegin 

\DIFadd{In the previous sections we showed that the communities emerging from the different weightings of the structural network quantitatively differ both at the macroscopic and at the microscopic scale. Here, we present some concrete examples of different community membership, in order to provide a practical illustration of the utility of using a multifaceted approach to community detection}\DIFaddend . \DIFaddbegin \DIFadd{We explore this first on a community level and then at the individual level.
}\DIFaddend 

In the topic-based communities, we \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{found }\DIFaddend a single community consisting of 83 users who tweet about environmental issues and frequently use hashtags such as \#green, \#eco and \#sustainability. We also \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{found }\DIFaddend a different community of 47 users who tweet about small businesses and entrepreneurship, using hashtags such as \#smallbiz, \#marketing and \#enterpreneur. In both cases\DIFdelbegin \DIFdel{most members of the }\DIFdelend \DIFaddbegin \DIFadd{, almost all members of these }\DIFaddend topic-based communities are not found in the same community in the other networks \DIFaddbegin \emph{\DIFadd{e.g.,}} \DIFadd{interaction- or activity-based}\DIFaddend , indicating that while these people talk about the same things and can therefore be considered a community based on their content, they do not strongly interact with each other nor behave the same, and so belong to different social groups with respect to interactions and behavior. \DIFaddbegin \DIFadd{This illustrates that the topics discussed by these users only define one facet of the users complex social network. 
}\DIFaddend 
...
\DIFdelbegin \DIFdel{As an aside, it }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend is interesting to examine \DIFdelbegin \DIFdel{who the most influential users are in these communities, as determined by transfer entropy. Recall that influence in this context refers to a }\DIFdelend \DIFaddbegin \DIFadd{the intersection between this topic-based community and the overlapping activity-based communities and explore which users in the intersection of these community types have the highest APC. Users with the highest APC in an activity-based community would provide the highest }\DIFaddend reduction in uncertainty \DIFdelbegin \DIFdel{about a follower(s) activity on Twitter given the tweet history of a followee. Said differently, higher transfer entropy, or a higher reduction in uncertainty, implies more influence. }\DIFdelend \DIFaddbegin \DIFadd{on average about that communities activity or inactivity on Twitter. }\DIFaddend Interestingly, among the \DIFdelbegin \DIFdel{most influential }\DIFdelend \DIFaddbegin \DIFadd{highest APC }\DIFaddend users in these \DIFdelbegin \DIFdel{communities, those }\DIFdelend \DIFaddbegin \DIFadd{communities---those }\DIFaddend with highest transfer entropy on \DIFdelbegin \DIFdel{average, we find }%DIFDELCMD < \@%%%
...
\DIFdelend \DIFaddbegin \DIFadd{$@$}\DIFaddend ConnectColorado, a page created to connect Coloradans, and the CBS Denver account, a popular news agency. This means that these accounts \DIFdelbegin \DIFdel{had high influence }\DIFdelend \DIFaddbegin \DIFadd{have high activity-based predictive capacity }\DIFaddend in terms of their followers' activity on Twitter. This is not surprising as these \DIFdelbegin \DIFdel{communities discuss Colorado }\DIFdelend \DIFaddbegin \DIFadd{activity-based communities heavily overlapped with the topic-based communities that discuss Colorado (mentioned above) }\DIFaddend and these accounts provide information on this topic. \DIFdelbegin \DIFdel{However, this does provide a check that this measure of activity-based influence }\DIFdelend \DIFaddbegin \DIFadd{This provides a brief proof of concept that the APC measure }\DIFaddend makes sense and suggests this could be a useful measure for \DIFdelbegin \DIFdel{influence on social media if used with other measures. This is discussed more in future work }\DIFdelend \DIFaddbegin \DIFadd{identifying individuals who possess high activity-based predictive capacity for a communities activity on a social media network. However, more work needs to be done to quantify this more fully}\DIFaddend .
