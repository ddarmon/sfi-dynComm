...
\begin{itemize}
	...
	\item \textbf{Activity-based:} Whom do you act like?
...
The activity-based approach is motivated by the question of which individuals act in a homogeneous manner, e.g., which users use a service in a similar way. The main tools for answering this question stem from information theory. For this approach, we consider each user on Twitter as an information processing unit, but ignore the content of their tweets, and weight the directed edges between users with the so-called \emph{transfer entropy}. 
This activity-based approach was motivated by a methodology originally used to detect functional communities within populations of neurons~\cite{shalizi2007discovering}. Information theoretic approaches have been used for data arising from online social networks to gain insight into local user behavior~\cite{darmon2013understanding}\alert{using ....}, to detect communities based on \emph{undirected} information flow~\cite{darmon2013detecting} using mutual information, and to perform link detection\alert{Dave, is this right or is it network inference and link prediction}~\cite{ver2012information}. 




While various information theoretic measures has been applied previously to the analysis of networks arising in online social media~\cite{ver2012information,darmon2013detecting}, to the best of our knowledge this is the first use of transfer entropy for community detection in social media networks. Transfer Entropy was originally proposed in~\cite{schreiber2000measuring} as an information theoretic measure of \emph{directed} information flow.  Transfer entropy is often thought of as being a measure of \emph{influence} in an information theoretic sense; this is because if the transfer entropy between a user $u$ and a user $v$ is positive, then this relationship is considered causal in the Granger sense~\cite{granger1963economic}. Effectively, if there is a positive transfer entropy from user $u$ to $v$ then there is a reduction in uncertantiy about user \alert{$u$/$v$} tweeting given the recent tweet history of user \alert{$u$/$v$}. This can easily be interpreted as a form of influence, if user \alert{$u$/$v$} reduces the uncertainty about \alert{$u$/$v$} tweeting then it may be the case that this was a form of social influence. We are fully aware that the difference between information theoretic and social influence are different but think this may be an interesting area to explore. Later we show that using this measure alone we can detect ``influential users" who are also listed as \alert{Forbes ...}. We instead treat this weighting as a way to quantify which users activity are quantified by each other, i.e., communities that use twitter in similar ways.
