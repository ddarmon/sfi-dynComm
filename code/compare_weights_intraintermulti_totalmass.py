...
for weight_type in ['TE6']:
	...
	for label_type in ['TE6']:
		...
		inter_in = collections.defaultdict(list)
		inter_out = collections.defaultdict(list)
		...
		for comm in inter_in.keys():
			with open('../data/edges-by-type/{}_comm{}_inter_in.dat'.format(output_prefix, comm), 'w') as wfile:
				for weight in inter_in[comm]:
					wfile.write('{}\n'.format(weight))
		
		for comm in inter_out.keys():
			with open('../data/edges-by-type/{}_comm{}_inter_out.dat'.format(output_prefix, comm), 'w') as wfile:
				for weight in inter_out[comm]:
					wfile.write('{}\n'.format(weight))
		
		for comm in intra.keys():
			with open('../data/edges-by-type/{}_comm{}_intra.dat'.format(output_prefix, comm), 'w') as wfile:
				for weight in intra[comm]:
					wfile.write('{}\n'.format(weight))
