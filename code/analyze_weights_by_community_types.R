...
for (comm_label in comms){
	...
	if (weight_type == 'mention-retweet'){
		...
		if (num.nonzero_i.to.i != length(data_i.to.i)){
			data_i.to.i = data_i.to.i[-which(data_i.to.i == 0)]
		}

		if (num.nonzero_e.to.i != length(data_e.to.i)){
			data_e.to.i = data_i.to.i[-which(data_e.to.i == 0)]
		}

		if (num.nonzero_i.to.e != length(data_i.to.e)){
			data_i.to.e = data_i.to.e[-which(data_e.to.i == 0)]
		}
