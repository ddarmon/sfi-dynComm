...
if (weight_type == 'TE4'){
	...
	xlim = range(c(data_inter, data_intra))
}
...
fs = c(density(data_inter, from = 0)$y, density(data_intra, from = 0)$y)
...
cat(sprintf('%s\t%s\t%f\t%g\t%g\t%g\n', label_type, weight_type, median(data_intra), median(data_inter), length(data_intra), length(data_inter)), file = output.filename, append = TRUE)
